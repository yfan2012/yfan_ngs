...
rawdir=/mithril/Data/NGS/Raw/211018_dunlop_nathan
projdir=/mithril/Data/NGS/projects/dunlop_insert
datadir=$projdir/run4
...
if [ $1 == trim ] ; then
    ...
    for i in NT350 NT351 NT358 NT359 ;
    do
...
if [ $1 == yield ] ; then
    ...
    for i in NT350 NT351 NT358 NT359 ;
    do
	bash ~/Code/utils/qc/basic_run_assess_zip.sh $rawdir/${i}*_L001_R1_001.fastq.gz >> $datadir/run4_info.csv
	bash ~/Code/utils/qc/basic_run_assess_zip.sh $rawdir/${i}*_L001_R2_001.fastq.gz >> $datadir/run4_info.csv
    done
    for i in NT350 NT351 NT358 NT359 ;
    do
	bash ~/Code/utils/qc/basic_run_assess_zip.sh $datadir/trimmed/${i}_fwd_paired.fq.gz >> $datadir/run4_info.csv
	bash ~/Code/utils/qc/basic_run_assess_zip.sh $datadir/trimmed/${i}_rev_paired.fq.gz >> $datadir/run4_info.csv
    done

    sed -i -e "s|$rawdir/||g" $datadir/run4_info.csv
    sed -i -e "s|$datadir/trimmed/||g" $datadir/run4_info.csv
