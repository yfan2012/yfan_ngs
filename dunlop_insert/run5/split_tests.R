...
countsfrac=allcounts %>%
    ...
    mutate(lightfrac=light/total) %>%
    arrange(-total) %>%
    arrange(-lightfrac)
countscsv=file.path(dbxdir, 'light_fraction.csv')
write_csv(countsfrac, countscsv)

countspdf=file.path(dbxdir, 'light_fraction.pdf')
pdf(countspdf, h=3, w=50)
plot=ggplot(countsfrac, aes(x=split, y=lightfrac, alpha=.2)) +
